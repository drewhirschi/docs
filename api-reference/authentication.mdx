...
const makeAuthenticatedRequest = async () => {
  const response = await fetch('https://app.enzo.health/api/v1/workflow_runs/workflow_run_1234', {
    method: 'GET',
    headers: {
      'Authorization': 'Bearer YOUR_API_TOKEN',
      'Content-Type': 'application/json',
      'Enzo-Api-Version': '2024-11-14'
    }
  });
...
```bash
curl -X GET https://app.enzo.health/api/v1/workflow_runs \
  -H "Authorization: Bearer YOUR_API_TOKEN" \
  -H "Content-Type: application/json" \
  -H "Enzo-Api-Version: 2024-11-14"
  -d '{ ... }'
```
